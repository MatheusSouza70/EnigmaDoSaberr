...
void Creditos() {
    ...
    al_draw_text(fonte, al_map_rgb(255, 255, 255), tela_largura / 2,
        tela_altura / 2.8, ALLEGRO_ALIGN_CENTER, "Eric Donato Pulcino dos Santos");
    al_draw_text(fonte, al_map_rgb(255, 255, 255), tela_largura / 2,
...
bool Collide(const Rect* rect1, const Rect* rect2, const Obstacle* obstacles, int numObstacles) {
...
int main() {
    ...
    ALLEGRO_BITMAP* npcSprite = al_load_bitmap("./quintas.png");
    if (!npcSprite) {
    ...
    const char* Dialogo_npc3[] = {
        "Eai! Firmeza?", "Você pode me ajudar?",
        "Posso pedir uma ajuda sua? Preciso resolver uma conta! "
        "Conto com você.",
        "Muito obrigado!" };
    ...
    int num_linhas3 = sizeof(Dialogo_npc3) / sizeof(Dialogo_npc3[0]);
    ...
    player->largura = 50;
    player->altura = 50;
    player->spritesheet = al_load_bitmap("./barradevida.png");
    ...
    npc2->imagem = al_load_bitmap("./quintas.png");
    npc2->x = 975;
    npc2->y = 525;
    npc2->largura = 50;
    npc2->altura = 50;

    npc3 = (Objeto*)malloc(sizeof(Objeto));
    npc3->imagem = al_load_bitmap("./quintas.png");
    npc3->x = 975;
    npc3->y = 300;
    npc3->largura = 50;
    npc3->altura = 50;
    ...
    al_draw_filled_rectangle(npc3->x, npc3->y, npc3->x + npc3->largura,
        npc3->y + npc3->altura,
        al_map_rgb(255, 0, 0)); // npc3
    ...
    while (!sair) {
        ...
        if (EstadoTela == menuprincipal) {
            ...
            // Verificação de colisão entre o jogador e npc3
            bool colisao_npc3 = (player->x < npc3->x + npc3->largura &&
                player->x + player->largura > npc3->x &&
                player->y < npc3->y + npc3->altura &&
                player->y + player->altura > npc3->y);
            ...
            largura_sprite_hp = al_get_bitmap_width(player->spritesheet);
            altura_sprite_hp = al_get_bitmap_height(player->spritesheet);

            al_draw_scaled_bitmap(npcSprite, 0, 0, al_get_bitmap_width(npcSprite),
                al_get_bitmap_height(npcSprite), npc->x, npc->y, 45,
                55, 0);
            al_draw_scaled_bitmap(npcSprite, 0, 0, al_get_bitmap_width(npcSprite),
                al_get_bitmap_height(npcSprite), npc2->x, npc2->y,
                45, 55, 0);
            al_draw_scaled_bitmap(npcSprite, 0, 0, al_get_bitmap_width(npcSprite),
                al_get_bitmap_height(npcSprite), npc3->x, npc3->y,
                45, 55, 0);
            ...
            if (colisao && EstadoTela == jogando) {
                if (tecla_e_pressionada) {
                    ...
                    if (dialogo_atual < 3) {
                        ...
                        al_flip_display();
                        dialogo_atual++;
                        al_rest(0.5); // Espera por 2 segundos entre cada parte do diálogo
                    }
            ...
            if (colisao_npc2 && EstadoTela == jogando) {
                if (tecla_e_pressionada) {
                    ...
                    if (dialogo_atual2 < 3) {
                        ...
                        al_flip_display();
                        dialogo_atual2++;
                         // Espera por 2 segundos entre cada parte do diálogo
                    }

                    else if (dialogo_atual2 >= 3) {
                        dialogo_atual2 = 0;
                    }

                    else if (dialogo_atual2 >= 3 && !jogo_finalizado2) {
                        // Reseta o diálogo quando terminar e muda o estado do jogo
                        dialogo_atual2 = 0;
                        EstadoTela = puzzle2;
                    }

                    else if (dialogo_atual2 < num_linhas2 && jogo_finalizado == true) {
                        al_draw_filled_rectangle(0, tela_altura / 1.3, tela_largura,
                            tela_altura, al_map_rgb(0, 0, 200));
                        al_draw_filled_rectangle(3, tela_altura / 1.3 + 3, tela_largura - 3,
                            tela_altura - 3, al_map_rgb(0, 0, 0));
            ...
            if (colisao_npc3 && EstadoTela == jogando) {
                if (tecla_e_pressionada) {
                    // Se a tecla E foi pressionada, exibe o próximo trecho do diálogo
                    if (dialogo_atual3 < 3) {
                        al_draw_filled_rectangle(0, tela_altura / 1.3, tela_largura,
                        ...
                        al_draw_text(fonte, al_map_rgb(255, 255, 255), 13,
                            tela_altura / 1.3 + 13, ALLEGRO_ALIGN_LEFT,
                            Dialogo_npc3[dialogo_atual3]);
                        al_flip_display();
                        dialogo_atual3++;
                        // Espera por 2 segundos entre cada parte do diálogo
                    }

                    else if (dialogo_atual3 >= 3 && !jogo_finalizado3) {
                        // Reseta o diálogo quando terminar e muda o estado do jogo
                        dialogo_atual3 = 0;
                        EstadoTela = puzzle2;
                    }

                    else if (dialogo_atual3 < num_linhas2 && jogo_finalizado == true) {
                        al_draw_filled_rectangle(0, tela_altura / 1.3, tela_largura,
                            tela_altura, al_map_rgb(0, 0, 200));
                        al_draw_filled_rectangle(3, tela_altura / 1.3 + 3, tela_largura - 3,
                            tela_altura - 3, al_map_rgb(0, 0, 0));
                ...
                colisao_anterior = colisao_npc3;
            }
            
        }

        else if (EstadoTela == controles) {
            // Código para mostrar informações de controles
        }
        else if (EstadoTela == creditos) {
            // Código para mostrar informações de créditos
        }
